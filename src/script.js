...
THREE.PlayerControls = function (camera, player, domElement) {
  ...
  this.jump = function () {
    ...
    //   player.position.y = 1 + Math.abs(Math.sin(elapsedTime * 3)) * 3;
    //   if (player.position.y < 1.05) playerIsJumping = false;
    // }

    if (playerIsJumping.action) {
      if (playerIsJumping.upPhase && player.position.y.toFixed(1) < playerIsJumping.startPos + 5) {
        player.position.y += 0.1;
        console.log(player.position.y.toFixed(2) )
      }
      if (playerIsJumping.upPhase && +player.position.y.toFixed(1)  === playerIsJumping.startPos + 5) {
        playerIsJumping.upPhase = false;
        playerIsJumping.downPhase = true;
      }
      if (playerIsJumping.downPhase && +player.position.y.toFixed(1) !== playerIsJumping.startPos) {
        player.position.y -= 0.1;
      }
      if (playerIsJumping.downPhase && +player.position.y.toFixed(1)  === playerIsJumping.startPos ) {
